...
def calculate_dtw_pearson(df_lead, df_follow, window_start, window_end):
    ...
    if len(sub_lead) < 5 or len(sub_fol) < 5: return 0
        
    s1 = sub_lead['rise'].values
    s2 = sub_fol['rise'].values
    
    # 🟢 1.8.5 優化：僅保留方向性初篩。若傳統相關係數極低或為負，則無需浪費 CPU 算 DTW
    # 這裡門檻設為 0，代表只要方向不完全相反就進入 DTW 深算
    if np.corrcoef(s1, s2)[0, 1] < 0:
        return 0
    ...
    try:
        dist, path = fastdtw(s1_norm, s2_norm, dist=euclidean)
        a1 = [s1_norm[i] for i, j in path]
        a2 = [s2_norm[j] for i, j in path]
...
def process_group_data(stock_data_collection, wait_minutes, hold_minutes,
    ...
    for i, row in merged_df.iterrows():
        ...
        for item in trigger_list:
            ...
            if cond == 'limit_up':
                tracking_stocks.clear(); tracking_stocks.add(sym)
                ...
                if verbose: message_log.append((current_time_str, f"{YELLOW}{sym} 漲停觸發，開始等待{RESET}"))
            else: 
        ...
        if tracking_stocks:
            ...
            if leader != max_sym:
                ...
                if verbose: message_log.append((current_time_str, f"{Fore.MAGENTA}🚀 領漲替換觸發，重新監控新領漲{Style.RESET_ALL}"))
        ...
        if in_waiting_period:
            # 🟢 滾動相似度檢查 (直接剔除不合格者)
            window_start_t = max((datetime.combine(date.today(), first_condition_one_time.time()) - timedelta(minutes=15)).time(), time(9,0))
            to_remove = []
            ...
            if waiting_time >= wait_minutes:
                ...
                def _rise_peak_flat(sym: str, join_time: datetime) -> bool:
                    ...
                    return (later_max is None) or (later_max <= pkval)

                # 🟢 修正：剔除所有冗餘代碼，一次性乾淨篩選
                eligible = []
                for sym in filtered_stocks:
                    if sym == leader: continue
                    if not _vol_break(sym, first_condition_one_time): continue
                    if not _rise_peak_flat(sym, first_condition_one_time): continue
                    
                    rise_now = row.get(f'rise_{sym}')
                    if rise_now is None or not (-1 <= rise_now <= 6): continue
                ...
                if not eligible:
                    ...
                    if entry_p < 10: gap, tick = price_gap_below_50, 0.01
                    elif entry_p < 50: gap, tick = price_gap_50_to_100, 0.05
...
def process_live_trading_logic(
    ...
    for grp, gstat in group_positions.items():
        ...
        if leader_sym and "tracking" in gstat:
            window_start_live = max(time(9,0), (gstat["start_time"] - timedelta(minutes=15)).time())
            to_remove_live = []
    ...
    for grp in groups_ready:
        ...
        for sym, info in filtered_track.items():
            ...
            if not _vol_break(sym, gstat["start_time"]): continue
            if not _rise_peak_flat(sym, gstat["start_time"]): continue
            ...
            if not (-1 <= rise_now <= 6): continue
            entry_price = row_now.iloc[0]["close"]
            if entry_price > capital_per_stock * 1.5: continue
            ...
            try:
                ...
                if not is_day_trade_yes: continue
            except: continue
        ...
        elif entry_px < 50: gap, tick = price_gap_50_to_100, 0.05
...
class QuantMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("交易程式 1.8.5 - 當沖量化終端")
        ...
        def make_btn(text, callback, color="#2C3E50"):
            ...
            btn.setStyleSheet(f"QPushButton {{ background-color: {color}; color: white; font-size: 15px; padding: 12px; border-radius: 6px; font-weight: bold;}} QPushButton:hover {{ background-color: #34495E; }}")
        ...
        vbox.addWidget(make_btn("📊 計算平均過高", lambda: AverageHighDialog().exec_(), "#34495E"))
